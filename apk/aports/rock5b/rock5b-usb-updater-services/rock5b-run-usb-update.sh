...
PAYLOAD_FILE="/opt/rock5b-updater/nvme-image.img.zst"
PAYLOAD_SHA256="/opt/rock5b-updater/nvme-image.img.zst.sha256"
TARGET_NVME_DEVICE="${TARGET_NVME_DEVICE:-/dev/nvme0n1}"
ROOT_PARTLABEL_REQUIRED="${ROOT_PARTLABEL_REQUIRED:-rock5b-updater-rootfs}"
TARGET_WAIT_SECONDS="${TARGET_WAIT_SECONDS:-120}"

LOCK_DIR="/run/rock5b-usb-update.lock"
BOOT_DONE_MARKER="/run/rock5b-usb-update.done"

log() {
  echo "[rock5b-usb-updater] $*"
}

log "ROCK5B USB updater image active."
...
partition_has_updater_bootcfg() {
  ...
  probe_mnt="/run/rock5b-updater-probe.$$"
  mkdir -p "$probe_mnt"
...
preserve_target_apkovl() {
    local config_dev="" config_mnt="/run/rock5b-apkovl-preserve"
    config_dev="$(derive_partition_device_from_root "$TARGET_NVME_DEVICE" 1)" || return 0
...
disable_root_extlinux() {
  local work_mnt="/run/rock5b-boot-rw"
...
boot_probe_mnt="/run/rock5b-boot-probe"
