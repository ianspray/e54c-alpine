...
PAYLOAD_FILE="/opt/rock3b-updater/nvme-image.img.zst"
PAYLOAD_SHA256="/opt/rock3b-updater/nvme-image.img.zst.sha256"
TARGET_NVME_DEVICE="${TARGET_NVME_DEVICE:-/dev/nvme0n1}"
ROOT_PARTLABEL_REQUIRED="${ROOT_PARTLABEL_REQUIRED:-rock3b-updater-rootfs}"
TARGET_WAIT_SECONDS="${TARGET_WAIT_SECONDS:-120}"

LOCK_DIR="/run/rock3b-usb-update.lock"
BOOT_DONE_MARKER="/run/rock3b-usb-update.done"

log() {
  echo "[rock3b-usb-updater] $*"
}

log "ROCK3B USB updater image active."
...
partition_has_updater_bootcfg() {
  ...
  probe_mnt="/run/rock3b-updater-probe.$$"
  mkdir -p "$probe_mnt"
...
preserve_target_apkovl() {
    local config_dev="" config_mnt="/run/rock3b-apkovl-preserve"
...
disable_root_extlinux() {
  local work_mnt="/run/rock3b-boot-rw"
...
boot_probe_mnt="/run/rock3b-boot-probe"
