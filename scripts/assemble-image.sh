...
ROOTFS_PART_SIZE="${ROOTFS_PART_SIZE:-2G}"
OPT_PARTLABEL="${OPT_PARTLABEL:-opt}"
OPT_PART_SIZE="${OPT_PART_SIZE:-2G}"
DOCKER_PARTLABEL="${DOCKER_PARTLABEL:-docker}"
DOCKER_PART_SIZE="${DOCKER_PART_SIZE:-2G}"
...
DISKLESS_TMPFS_MIN_MIB="${DISKLESS_TMPFS_MIN_MIB:-0}"
DISKLESS_TMPFS_SIZE_MIB="${DISKLESS_TMPFS_SIZE_MIB:-}"

# Partition geometry (512-byte sectors):
# - p1 config: 256 MiB (starts at 16 MiB)
# - p2 bootcfg: 300 MiB
# - p3 rootfs: 2 GiB by default
# - p4 opt: 2 GiB by default
# - p5 docker: 2 GiB by default
# Total image: 7 GiB
P1_START=32768
...
CMDLINE_IMMUTABLE_DEFAULT="${BOARD_KERNEL_CMDLINE_IMMUTABLE_DEFAULT:-${CMDLINE_BASE} ro diskless=yes diskless_tmpfs_size=${DISKLESS_TMPFS_SIZE_MIB} apkovl=PARTLABEL=config:/alpian.apkovl.tar.gz}"
...
LABEL immutable
  MENU LABEL ${IMMUTABLE_MENU_LABEL}
  LINUX /boot/Image
