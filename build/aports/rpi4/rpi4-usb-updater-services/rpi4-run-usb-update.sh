...
PAYLOAD_FILE="/opt/rpi4-updater/nvme-image.img.zst"
PAYLOAD_SHA256="/opt/rpi4-updater/nvme-image.img.zst.sha256"
...
log() {
  echo "[rpi4-usb-updater] $*"
}

log "RPI4 USB updater image active."
...
partition_has_updater_bootcfg() {
  ...
  probe_mnt="/run/rpi4-updater-probe.$$"
  mkdir -p "$probe_mnt"
...
preserve_target_apkovl() {
    local config_dev="" config_mnt="/run/rpi4-apkovl-preserve"
