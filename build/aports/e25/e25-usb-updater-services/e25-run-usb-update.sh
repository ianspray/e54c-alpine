...
PAYLOAD_FILE="/opt/e25-updater/nvme-image.img.zst"
PAYLOAD_SHA256="/opt/e25-updater/nvme-image.img.zst.sha256"
TARGET_DEVICE="${TARGET_DEVICE:-/dev/mmcblk0}"
ROOT_PARTLABEL_REQUIRED="${ROOT_PARTLABEL_REQUIRED:-e25-updater-rootfs}"
...
log() {
  ...
  echo "[e25-usb-updater] $*" >/dev/console 2>/dev/null || true
  echo "[e25-usb-updater] $*"
}

log "E25 updater image active."
...
partition_has_updater_bootcfg() {
  ...
  probe_mnt="/run/e25-updater-probe.$$"
  mkdir -p "$probe_mnt"
...
preserve_target_apkovl() {
    local config_dev="" config_mnt="/run/e25-apkovl-preserve"
...
disable_root_extlinux() {
  local work_mnt="/run/e25-boot-rw"
...
boot_probe_mnt="/run/e25-boot-probe"
